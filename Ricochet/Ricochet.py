...
class Bullet:
    ...
    def draw(self, surface):
        pygame.draw.circle(surface, WHITE, self.pos, 5)  # 5 = bullet radius


def check_collision_with_target(bullet, target_position, target_radius):
    distance = (
        (bullet.pos[0] - target_position[0]) ** 2
        + (bullet.pos[1] - target_position[1]) ** 2
    ) ** 0.5
...
def is_hovering(mouse_pos, circle_pos, circle_radius):
    distance_squared = (circle_pos[0] - mouse_pos[0]) ** 2 + (
...
triangle_points = [left_triangle_point, right_triangel_point, top_trianlge_point]
...
while running:
    ...
    hovering_wall = (
        distance_point_to_line_segment(mouse_pos, wall_start_pos, wall_end_pos) <= 10
    )
    ...
    for event in pygame.event.get():
        ...
        elif event.type == pygame.MOUSEBUTTONDOWN and not is_edit_mode:
            bullet_velocity = [5, 0]
            bullets.append(Bullet(player_position, bullet_velocity))
        ...
        # Handling mouse click in edit mode
            ...
            # Check for bounce platform (triangle)
                ...
                triangle_offset = [substract(p, mouse_pos) for p in triangle_points]

            elif (
                distance_point_to_line_segment(mouse_pos, wall_start_pos, wall_end_pos)
                < 10
            ):
                dragging_wall = True
                ...
                if shift_pressed:
                    ...
                    if (mouse_pos[0] - wall_start_pos[0]) ** 2 + (
                        mouse_pos[1] - wall_start_pos[1]
                    ) ** 2 < (mouse_pos[0] - wall_end_pos[0]) ** 2 + (
                        mouse_pos[1] - wall_end_pos[1]
                    ) ** 2:
        ...
        # Handle mouse release
            ...
            and dragging_player
            and selected_object == "player"
        ):
            mouse_x, mouse_y = event.pos
            player_position = (mouse_x + player_offset_x, mouse_y + player_offset_y)
            ...
            mouse_x, mouse_y = event.pos
            target_position = (mouse_x + target_offset_x, mouse_y + target_offset_y)

        elif dragging_triangle and selected_object == "triangle":
            mouse_x, mouse_y = event.pos
            triangle_points = [add(p, (mouse_x, mouse_y)) for p in triangle_offset]
            ...
            if wall_drag_mode == "whole":
                ...
                wall_start_pos = (
                    original_wall_start_pos[0] + dx,
                    original_wall_start_pos[1] + dy,
                )
                wall_end_pos = (
                    original_wall_end_pos[0] + dx,
                    original_wall_end_pos[1] + dy,
                )
    ...
    # highlight player
    if is_edit_mode and is_hovering(
        pygame.mouse.get_pos(), player_position, player_radius
    ):
        pygame.draw.circle(screen, WHITE, player_position, player_radius + 2, 2)
    # highlight target
    elif is_edit_mode and is_hovering(
        pygame.mouse.get_pos(), target_position, target_radius
    ):
    ...
    # Highlight triangle
        ...
        if (
            point_in_triangle(pygame.mouse.get_pos(), *triangle_points)
            or selected_object == "triangle"
        ):
    ...
    if hovering_wall and is_edit_mode:
        ...
        pygame.draw.line(
            screen, wall_color, wall_start_pos, wall_end_pos, wall_thickness + 4
        )
    else:
        # Draw the wall normally
        pygame.draw.line(
            screen, wall_color, wall_start_pos, wall_end_pos, wall_thickness
        )
    ...
    # Draw the wall
    pygame.draw.line(screen, wall_color, wall_start_pos, wall_end_pos, wall_thickness)
    ...
    for bullet in bullets[:]:
        ...
        # Triangle collision check
        if point_in_triangle(bullet.pos, *triangle_points):
            print("Bullet hit triangle!")
            # Handle collision...

        # Wall collision check
        if (
            distance_point_to_line_segment(bullet.pos, wall_start_pos, wall_end_pos)
            <= 5 + wall_thickness / 2
        ):  # Assuming 5 is the radius of the bullet
        ...
        if (
            dist_to_target <= 5 + target_radius
        ):  # Assuming 5 is the radius of the bullet
            print("Bullet hit the target!")
            # Handle collision...

        # if bullet of screen remove it

        if (
            bullet.pos[0] < 0
            or bullet.pos[0] > screen_width
            or bullet.pos[1] < 0
            or bullet.pos[1] > screen_height
        ):
